...
%
% 1. HRTF_1_or_angle = 'hrtf b_nh2.sofa';  % ARI database
%    or
%    HRTF_1_or_angle = [azi ele]; % angle matrix
%
% 2. HRTF_2_or_angle = 'subject_009.sofa';  % CIPIC database
%    or
%    HRTF_2_or_angle = [azi ele]; % angle matrix
%
% 3. range = 2 % matching range
% 4. plot = 1 % plot trigger, 1 = plot, 0 = no plot, 'ture' = real distance
...
%
% 5. method = 'max' % matching method, 'max', 'min','distance max', 'distance min'  or 'fast'
...
%
% 6. print = 0 % if 0: no progress will be shown in command window (advance purpose)
%
%
% OUTPUT:
% 1. HRTF_1_common_angles  % matched angles in hrtf 1 
% 2. HRTF_2_common_angles  % matched angles in hrtf 2 
%    - (cloumn 1: azimuth angle, 2: elevation angle, 3: distance (not useful))
%
% 3. HRTF_data_desire   % matches between hrtf 1 and hrtf 2 in chosen method(e.g. min)
% 4. HRTF_data_opposite   % matches between hrtf 1 and hrtf 2 in opposite method(e.g. max)
